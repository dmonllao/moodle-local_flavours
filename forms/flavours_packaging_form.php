...
class flavours_packaging_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('text', 'author', get_string('flavourauthor', 'local_flavours'), 'maxlength="154" size="40"');
        $mform->setType('author', PARAM_TEXT);
        $mform->setDefault('author', $USER->firstname.' '.$USER->lastname);

        // Ingredients
        $mform->addElement('header', 'ingredients', get_string('selectingredients', 'local_flavours'));
        $mform->addElement('html', '<div id="id_ingredients_tree" class="ygtv-checkbox">'.$this->_customdata["treedata"].'</div>');
    

        $mform->addElement('hidden', 'action', 'packaging_execute');
        $mform->addElement('submit', 'ingredients_submit', get_string('packageflavour', 'local_flavours'));
    }
}
